...
func (s *Scanner) matchFileDirect(ctx context.Context, tf *database.TrackFile, tags *tagreader.Tags) (bool, error) {
...
func (s *Scanner) matchFileFuzzy(ctx context.Context, tf *database.TrackFile, tags *tagreader.Tags) (bool, error) {
...
func (s *Scanner) DeleteTrackFile(ctx context.Context, trackFileID int64) error {
	tf, err := s.db.GetTrackFile(ctx, trackFileID)
	if err != nil {
		return fmt.Errorf("get track file: %w", err)
	}
	...
	return s.db.DeleteTrackFile(ctx, trackFileID)
}
